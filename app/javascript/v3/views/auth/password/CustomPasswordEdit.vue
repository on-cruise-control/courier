...
export default {
  ...
  data() {
    return {
      credentials: { confirmPassword: '', password: '' },
      newPasswordAPI: { message: '', showLoading: false },
      error: '',
    };
  },
  computed: {
  ...
  validations: {
    credentials: {
      ...
      confirmPassword: {
        ...
        isEqPassword(value) {
          if (value !== this.credentials.password) return false;
          return true;
        },
      },
    },
      ...
      <div class="flex-1 flex flex-col justify-center max-w-sm mx-auto w-full mt-4 sm:mt-8">
        ...
        <form class="space-y-4" @submit.prevent="submitForm">
          ...
          <button
            ...
            "
          >
            <Spinner v-if="newPasswordAPI.showLoading" size="" color-scheme="white" />
            <span v-else>{{ $t('SET_NEW_PASSWORD.SUBMIT') }}</span>
          </button>
        </form>

class Sms::ServiceEscalationNotificationService
  ...
  def perform
    ...
    Rails.logger.error("Failed to send service escalation SMS notifications: #{e.message}")
  end
  ...
  def build_message_body
    ...
    body = <<~SMS
      🚨 Urgent Service Escalation Required
      ...
      A service-related customer complaint. Please take over this conversation manually.
    SMS
  ...
  def send_sms_to_recipients(recipients, message_body)
    ...
    recipients.each do |recipient|
      ...
      Rails.logger.error("Failed to send service escalation SMS to #{recipient.name} (#{recipient.phone_number}): #{e.message}")
    rescue StandardError => e
      Rails.logger.error("Unexpected error sending service escalation SMS to #{recipient.name}: #{e.message}")
    end
  end
end

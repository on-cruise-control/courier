...
RSpec.describe Sms::NegativeSentimentEscalationService do
  ...
  let!(:user) { create(:user, email: 'negative@example.com', phone_number: '+1234567890') }

  let(:service) do
    described_class.new(
      conversation: conversation,
      emails: ['negative@example.com']
    )
  end
  ...
  describe '#perform' do
    context 'when sms config is valid and there are recipients' do
      it 'sends negative sentiment sms' do
        expect(messages).to receive(:create).with(
          from: '+1098765432',
          to: '+1234567890',
          body: include('Negative Comment Detected').and(include('John Doe'))
        )
        service.perform
      end
    end

...
RSpec.describe Sms::ServiceEscalationNotificationService do
  ...
  let!(:user) { create(:user, email: 'service-lead@example.com', phone_number: '+1234567890') }

  let(:service) { described_class.new(conversation: conversation, emails: ['service-lead@example.com']) }
  ...
  describe '#perform' do
    it 'sends the service escalation sms' do
      expect(messages).to receive(:create).with(
        from: '+1098765432',
        to: '+1234567890',
        body: include('Urgent Service Escalation Required').and(include('John Doe'))
      )
      service.perform
    end

...
RSpec.describe Sms::TeamReachedOutNotificationService do
  ...
  let!(:user) { create(:user, email: 'team@example.com', phone_number: '+1234567890') }
  ...
  describe '#perform' do
    context 'when sms config is valid and there are recipients' do
      it 'sends team reached out sms' do
        expect(messages).to receive(:create).with(
          from: '+1098765432',
          to: '+1234567890',
          body: include('Follow-Up Required: Team Has Not Reached Out').and(include('John Doe'))
        )
        service.perform
      end
    end

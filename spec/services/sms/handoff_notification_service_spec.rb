...
RSpec.describe Sms::HandoffNotificationService do
  ...
  let!(:user) { create(:user, email: 'handoff@example.com', phone_number: '+1234567890') }

  let(:service) do
    described_class.new(
      conversation,
      emails: ['handoff@example.com']
    )
  end

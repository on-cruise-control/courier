...
RSpec.describe AgentNotifications::SalesEscalationMailer do
  let(:class_instance) { described_class.new }
  let!(:account) { create(:account) }
  let(:agent) { create(:user, email: 'sales-lead@example.com', account: account) }
  ...
  describe 'escalation_notification' do
    ...
    it 'mentions the sales department in the body' do
      expect(mail.body.encoded).to include('sales department')
    end
    ...
    context 'with an incoming message' do
      let(:mail) do
        described_class.with(account: account).escalation_notification(
          emails: [agent.email], conversation: conversation, customer_data: nil, message: 'The salesman lied to me'
        ).deliver_now
      end

      it 'includes the message content' do
        expect(mail.body.encoded).to include('The salesman lied to me')
      end
    end
  end
end

...
RSpec.describe VehiclePartsEscalationNotificationJob do
  let(:account) { create(:account) }
  let(:inbox) { create(:inbox, account: account) }
  let(:contact) { create(:contact, account: account) }
  let(:conversation) { create(:conversation, account: account, inbox: inbox, contact: contact) }
  let(:emails) { ['parts-lead@example.com'] }
  let(:customer_data) { { 'name' => 'John Doe', 'phone' => '+1234567890', 'email' => 'john@example.com' } }
  let(:message) { create(:message, conversation: conversation, content: 'I ordered brake pads weeks ago and still nothing') }

  let(:summary_service) { instance_double(Conversations::SummaryService) }
  let(:sms_service) { instance_double(Sms::VehiclePartsEscalationNotificationService) }
  ...
  before do
    ...
    allow(AgentNotifications::VehiclePartsEscalationMailer).to receive(:escalation_notification).and_return(mailer_double)
    allow(mailer_double).to receive(:deliver_now)

    allow(Sms::VehiclePartsEscalationNotificationService).to receive(:new).and_return(sms_service)
  ...
  describe '#perform' do
    it 'refreshes the summary, sends the mailer and the sms' do
      ...
      expect(AgentNotifications::VehiclePartsEscalationMailer).to have_received(:escalation_notification).with(
        emails: emails, conversation: conversation, customer_data: customer_data, message: message
      )
      expect(mailer_double).to have_received(:deliver_now)
      expect(Sms::VehiclePartsEscalationNotificationService).to have_received(:new).with(
